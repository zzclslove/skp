...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'allgoodsprice')
{
    ...
    foreach($goods as $good){
        ...
        foreach($zharray as $val){
            ...
            $shop_price = intval($shop_price * 6.6716);
            $html .= '<tr>';
            $html .= '<td>'.(strlen(trim($good['seller_note'])) == 0?$good['goods_name']:$good['seller_note']).'</td>';
            $html .= '<td>'.$attr_str.'</td>';
...
$discount = $_SESSION['discount'] == 1 ? '' : '(' . (1 - $_SESSION['discount'])*100 . '% off)';
